      ...
      <main className="min-h-screen bg-ui-surface py-10">
        <div className="max-w-6xl mx-auto px-6">
          ...
          {isLoading ? (
            ...
            const activeRounds = rounds.filter(r => !(r.duration && r.duration !== '00:00:00'));
            const completedRounds = rounds.filter(r => !!(r.duration && r.duration !== '00:00:00'));

            const RoundCard = ({ r }: { r: typeof rounds[0] }) => {
              const isComplete = !!(r.duration && r.duration !== '00:00:00');
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-3 pr-7">
                    ...
                    {isComplete ? (
                      <span className="text-[10px] font-bold text-ui-variant bg-ui-low px-2 py-0.5 rounded-full">완료</span>
                    ) : (
                      <span className="text-[10px] font-bold text-white bg-brand-primary px-2 py-0.5 rounded-full uppercase">진행중</span>
                    )}
                  </div>
            ...
            return (
              <>
                {activeRounds.length > 0 && (
                  <div className="mb-8">
                    <p className="text-xs font-bold text-ui-variant uppercase tracking-wider mb-3">진행중 ({activeRounds.length})</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {activeRounds.map(r => <RoundCard key={r.id} r={r} />)}
                    </div>
                  </div>
                )}
                  ...
                  <div>
                    {activeRounds.length > 0 && (
                      <div className="flex items-center gap-3 mb-3">
                        <div className="flex-1 h-px bg-ui-high/40" />
                        <p className="text-xs font-bold text-ui-variant uppercase tracking-wider">완료 ({completedRounds.length})</p>
                        <div className="flex-1 h-px bg-ui-high/40" />
                      </div>
                    )}
                    {activeRounds.length === 0 && (
                      <p className="text-xs font-bold text-ui-variant uppercase tracking-wider mb-3">완료 ({completedRounds.length})</p>
                    )}
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 opacity-75">
                      {completedRounds.map(r => <RoundCard key={r.id} r={r} />)}
                    </div>
                  </div>
                )}
              </>
